...
data = pd.read_csv("COVID-19-geographic-disbtribution-worldwide.csv", delimiter=";", parse_dates=["dateRep"],
...
for i in range(0, len(CountryData)):
	...
	deathspercentoftotal.append(CountryData.iloc[i]["deaths"] /
								data[data["countriesAndTerritories"] == CountryData.iloc[i]["countriesAndTerritories"]][
									"popData2018"].max())
	# Wie viel Prozent der Bevölkerung sind infiziert. (Fallzahlen / Bevölkerung)
	casepercent.append(CountryData.iloc[i]["cases"] /
					   data[data["countriesAndTerritories"] == CountryData.iloc[i]["countriesAndTerritories"]][
						   "popData2018"].max())
...
CountryData = CountryData.loc[
	CountryData["deathspercentoftotal"] < CountryData["deathspercentoftotal"].quantile(q=0.95)]
...
for i in range(0, len(deathspercentbins["deathspercentbins"])):
	labels.append(
		str(deathspercentbins["deathspercentbins"][i]).replace(",", "\n").replace("(", "").replace(")", "").replace("]",
																													"").replace(
			"[", ""))
...
for i in range(0, xtickspos - 1):
	labels2.append(
		str(casepercentbins["casepercentbins"][i * xtickspos]).lstrip("()[].1234567890-e ").replace(",", "").replace(
			"(", "").replace(")", "").replace("]", "").replace("[", ""))
labels2.append(
	str(casepercentbins["casepercentbins"].iloc[-1]).lstrip(" ()[].1234567890-e").replace(",", "").replace("(",
																										   "").replace(
		")", "").replace("]", "").replace("[", ""))
...
plt.scatter(PastWeek[PastWeek["countriesAndTerritories"] == "Germany"]["cases"],
			PastWeek[PastWeek["countriesAndTerritories"] == "Germany"]["caseslastweek"], label="Germany", color="green")
plt.scatter(PastWeek[PastWeek["countriesAndTerritories"] == "United_States_of_America"]["cases"],
			PastWeek[PastWeek["countriesAndTerritories"] == "United_States_of_America"]["caseslastweek"], label="USA")
...
plt.yscale('log')
# Beschriftungen
plt.title("Countries COVID-19 " + "(" + str(data.sort_values(["dateRep"], ascending=False, ).iloc[0]["dateRep"]).rstrip(
	"0:") + ")")
...
plt.plot(data[data["countriesAndTerritories"] == "United_States_of_America"]["dateRep"], casesagg2,
